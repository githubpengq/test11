...
package com.cotton.web.servlet;
...
import com.cotton.service.proxy.ProxyTargert;
...
public class ProxyServlet extends HttpServlet {
  ...
  private ProxyTargert proxyTargert;
...
//    }
    ...
    this.proxyTargert=SpringContext.get(ProxyTargert.class);
  ...
  protected void initTarget(HttpServletRequest servletRequest) throws ServletException {
	    String uri=servletRequest.getRequestURI();
	    String host=this.proxyTargert.getTargetHostByPath(uri);
	    if (host == null){
	    	throw new ServletException("host is null. uri:"+uri);
	    }
	    targetUri=host+uri;
	    if (targetUri == null)
	      throw new ServletException(P_TARGET_URI+" is required.");
  ...
  protected void service(HttpServletRequest servletRequest, HttpServletResponse servletResponse)
		...
		String uri = servletRequest.getRequestURI();
		if (uri.startsWith(Constant.URI_AUTH_PREFIX)) {
			String uriPath=uri.replace(".json", ".jsonx");
			logger.warn("服务器跳转 "+uriPath);
			servletRequest.getRequestDispatcher(uriPath).forward(servletRequest,servletResponse);
			return;
		}
		initTarget(servletRequest);
		...
		//请求中添加参数 --20170825
		proxyRequestUri=addParamForProxyUrl(proxyRequestUri,servletRequest);
  ...
  protected HttpResponse doExecute(HttpServletRequest servletRequest, HttpServletResponse servletResponse,
	...
	if (doLog) {
		    logger.info("代理 " + servletRequest.getMethod() + " uri " + servletRequest.getRequestURI() + " -- " +
  ...
  private static String addParamForProxyUrl(String url,HttpServletRequest req){
      String[] urlArr = url.split("[?]");
      Long userId=AuthBaseAction.getUserId(req);
      if (urlArr.length > 1) {
